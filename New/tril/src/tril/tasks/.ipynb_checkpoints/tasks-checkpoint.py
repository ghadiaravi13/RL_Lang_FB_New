...
from tril.base_task import BaseTask, PreferenceSample, Sample, OfflineSample
...
class TLDR(BaseTask):
    ...
    def prepare(
        ...
        n_samples: Dict[str, int] = {"train":500, "valid": 100, "test": 500},
    ):
...
class UltraFeedback(BaseTask):
    ...
    def prepare(
        ...
        tokenizer_id: str,
        max_prompt_length: int,
        max_ref_length: int = None,
        offline_RL: bool = False,
        n_samples: Dict[str, int] = {"train":500, "val": 100, "test": 500},
    ):
        tokenizer = AutoTokenizer.from_pretrained(
            tokenizer_id
        )  # NOTE: truncation side | right, padding side | left
        ...
        def filter_longer_samples(x):
            prompt = tokenizer(x['instruction'], add_special_tokens=False)['input_ids']
            response = tokenizer(x['response'])['input_ids']
            return len(prompt) <= max_prompt_length and len(response) <= max_ref_length
            
        split_name = UltraFeedback.gen_split_name(split)
        ds = pd.read_csv(f'/home/jupyter/Ravi_new/RL_Language_Feedback/New/UltraFeedback/{split_name}_data.csv').dropna().reset_index()
        ds = ds[ds.apply(filter_longer_samples,axis=1)]
        ds['idx'] = ds.index
        ds = ds.reset_index()
        samples = []
        ...
        for i in tqdm(
            ...
            if offline_RL:
                ...
                id=ds.iloc[i]['idx'],
                prompt_or_input_text=ds.iloc[i]['instruction'],
                references=[ds.iloc[i]['response']],
                reward=int(ds.iloc[i]['helpfulness_rating']),
                meta_data={"reference": ds.iloc[i]['response']},
            )
            else:
                sample = Sample(
                    id=ds.iloc[i]['idx'],
