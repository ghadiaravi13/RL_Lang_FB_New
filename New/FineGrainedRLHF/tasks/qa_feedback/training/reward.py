...
class SubSentenceVerbosityReward:
    ...
    def get_reward(self, 
        ...
        # print(len(generated_texts), len(batch_nf_reward_inputs), inputs['input_ids'].shape, batch_nf_pred.logits.shape)
...
class FineGrainedReward(BasicReward):
    ...
    def get_finegrained_reward(self, prompts_input_ids, prompts_attention_mask, 
        ...
        # combine the rewards
        verbosity_rew_given, factuality_rew_given, completeness_rew_given = [], [], []
        for text_idx, generated_text in enumerate(generated_texts):

            coeff1, coeff2, coeff3 = 1,1,1
            if self.vector_RL>1:
                coeff1 = (1 if text_idx<(len(generated_texts)//self.vector_RL) else 0)
                coeff2 = (1 if (text_idx<(2*len(generated_texts)//self.vector_RL) and text_idx>=(len(generated_texts)//self.vector_RL)) else 0)
                coeff3 = (1 if text_idx>=(2*len(generated_texts)//self.vector_RL) else 0)
            fine_grained_reward = [(coeff1*a)+(coeff2*b)+(coeff3*c) for a,b,c in zip(verbosity_rewards[text_idx], 
                                                                                     factuality_rewards[text_idx], 
                                                                                     completeness_rewards[text_idx])]
            verbosity_rew_given.append([coeff1*a for a in verbosity_rewards[text_idx]])
            factuality_rew_given.append([coeff2*b for b in factuality_rewards[text_idx]])
            completeness_rew_given.append([coeff3*c for c in completeness_rewards[text_idx]])
        ...
        return {"rewards": fine_grained_rewards, 
                ...
                "verbosity_rew_given": verbosity_rew_given,
                "factuality_rew_given": factuality_rew_given,
                "completeness_rew_given": completeness_rew_given,
               }
    ...
    def get_reward(self, 
        ...
        return {'rewards/raw': rewards_output['rewards'],
                'rewards/verbosity': rewards_output['verbosity_rew_given'],
                'rewards/factuality': rewards_output['factuality_rew_given'],
                'rewards/completeness': rewards_output['completeness_rew_given'],
               }
    ...
    def eval_metrics(self, 
        ...
        output.update({
            ...
            "eval/verbosity_rew_given": [np.sum(sublist) for sublist in finegrained_rewards_output['verbosity_rew_given']],
            "eval/factuality_rew_given": [np.sum(sublist) for sublist in finegrained_rewards_output['factuality_rew_given']],
            "eval/completeness_rew_given": [np.sum(sublist) for sublist in finegrained_rewards_output['completeness_rew_given']],
