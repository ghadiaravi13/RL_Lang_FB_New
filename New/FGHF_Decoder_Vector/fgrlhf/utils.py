...
def whiten(values, masks, shift_mean=True, accelerator=None, vector_RL=1):
    if vector_RL==1:
        ...
        final_whitened = []
        for i in range(vector_RL):
            if accelerator is not None:
                all_values = accelerator.gather(values) # (num_gpus * B * vector_RL, KL)
                total_num_values = all_values.shape[0]
                all_masks = accelerator.gather(masks) # (num_gpus * B * vector_RL, KL)
                mean, var = reduce_mean(all_values[i*(total_num_values//vector_RL):(i+1)*(total_num_values//vector_RL)], all_masks[i*(total_num_values//vector_RL):(i+1)*(total_num_values//vector_RL)]), reduce_std(all_values[i*(total_num_values//vector_RL):(i+1)*(total_num_values//vector_RL)], all_masks[i*(total_num_values//vector_RL):(i+1)*(total_num_values//vector_RL)])
            else:
                total_num_values = values.shape[0]
                mean, var = reduce_mean(values[i*(total_num_values//vector_RL):(i+1)*(total_num_values//vector_RL)], masks[i*(total_num_values//vector_RL):(i+1)*(total_num_values//vector_RL)]), reduce_std(values[i*(total_num_values//vector_RL):(i+1)*(total_num_values//vector_RL)], masks[i*(total_num_values//vector_RL):(i+1)*(total_num_values//vector_RL)])
            # if accelerator is not None and accelerator.is_main_process:
            #     print(f'all_values: {all_values}, all_masks: {all_masks}')
            #     print(f'mean: {mean}, var: {var}')
            whitened = (values[i*(total_num_values//vector_RL):(i+1)*(total_num_values//vector_RL)] - mean) * torch.rsqrt(var + 1e-8)
            if not shift_mean:
                whitened += mean
            final_whitened.append(whitened)
        return torch.concat(final_whitened)
