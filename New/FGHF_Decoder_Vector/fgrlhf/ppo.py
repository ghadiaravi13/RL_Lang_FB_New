...
class PPOTrainer:
    ...
    def compute_advantages(self, results, num_samples, vector_RL=1):
        ...
        with torch.no_grad():
            if self.args['ppo']['whiten_rewards']:
                whitened_rewards = whiten(rewards, mask, shift_mean=False, accelerator=self.accelerator, vector_RL=vector_RL)
            else:
    ...
    def loss(self, results, all_mask_weight, rew_id):
    ...
    def train(self, step):
        ...
        try:
            ...
            self.train_sampler = iter(self.train_dataloader if self.train_dataloader is not None else {})
            batch = next(self.train_sampler)
        ...
        with torch.no_grad():
            reward_results = self.reward_model.get_reward(
                ...
                metadata = [elem for elem in batch['metadata'] for _ in range(self.args['env']['train_num_samples_per_input'])]*self.vector_RL,
            )
            results.update(reward_results)
            self.reward_model.kl_penalize_reward(results)

        # Get advantages
        self.compute_advantages(results, self.args['env']['train_num_samples_per_input'], self.vector_RL)
        
        n_results = len(results['generated_input_ids'])
        
        loss_totals, loss_policies, loss_values =  [], [], []
        reward_penalizeds, reward_kls, reward_raws, reward_verbositys, reward_factualitys, reward_completenesses = [], [], [], [], [], []
        ...
        for ppo_epoch_idx in range(self.args['train']['n_ppo_epoch_per_rollout']):
            ...
            for batch_idx in range(0,n_results, self.args['train']['training_batch_size_per_card']):
                ...
                if self.vector_RL>1:
                    if batch_idx < n_results // self.vector_RL:
                        rew_id = 0
                    elif batch_idx >= n_results // self.vector_RL and batch_idx < 2 * n_results // self.vector_RL:
                        rew_id = 1
                    elif batch_idx >=  2 * n_results // self.vector_RL:
                        rew_id = 2
                else:
                    rew_id = None
                ...
                if ppo_epoch_idx == self.args['train']['n_ppo_epoch_per_rollout'] - 1:
                    ...
                    reward_verbosity =  torch.mean(reduce_sum(batch_results['rewards/verbosity'], batch_results['generated_attention_mask'], axis=1)).unsqueeze(0) # (1)
                    reward_factuality =  torch.mean(reduce_sum(batch_results['rewards/factuality'], batch_results['generated_attention_mask'], axis=1)).unsqueeze(0) # (1)
                    reward_completeness =  torch.mean(reduce_sum(batch_results['rewards/completeness'], batch_results['generated_attention_mask'], axis=1)).unsqueeze(0) # (1)
                    ...
                    reward_verbositys.append(reward_verbosity)
                    reward_factualitys.append(reward_factuality)
                    reward_completenesses.append(reward_completeness)
            ...
            # self.accelerator.print(self.accelerator.unwrap_model(self.policy_model.model).lm_head.weight.grad.shape)
            # self.accelerator.print(self.accelerator.unwrap_model(self.policy_model.model).lm_head.weight.shape)
            # self.accelerator.print(torch.all(self.accelerator.unwrap_model(self.policy_model.model).lm_head.weight.grad==0))
            # self.accelerator.print(torch.all(self.accelerator.unwrap_model(self.policy_model.model).lm_head.weight.cpu()==self.policy_model.init_lm_head))
            self.optimizer.step()
            self.scheduler.step()
        ...
        reward_verbosity = torch.cat(reward_verbositys, dim=0)
        reward_factuality = torch.cat(reward_factualitys, dim=0)
        reward_completeness = torch.cat(reward_completenesses, dim=0)
        ...
        rewards_verbosity = self.accelerator.gather(reward_verbosity) # (num_gpus)
        rewards_factuality = self.accelerator.gather(reward_factuality) # (num_gpus)
        rewards_completeness = self.accelerator.gather(reward_completeness) # (num_gpus)
        ...
        reward_verbosity = rewards_verbosity.mean().item()
        reward_factuality = rewards_factuality.mean().item()
        reward_completeness = rewards_completeness.mean().item()

        # import pdb;pdb.set_trace()
        #tracking difference between lm_head weights for vector RL
        lm_head_default_dim = self.accelerator.unwrap_model(self.policy_model.model).lm_head.weight.shape[0]//self.vector_RL

        head_wt_0 = self.accelerator.unwrap_model(self.policy_model.model).decoder.vector_heads[0].layer[0].SelfAttention.q.weight
        head_wt_1 = self.accelerator.unwrap_model(self.policy_model.model).decoder.vector_heads[1].layer[0].SelfAttention.q.weight
        head_wt_2 = self.accelerator.unwrap_model(self.policy_model.model).decoder.vector_heads[2].layer[0].SelfAttention.q.weight

        init_head_wt_0 = self.policy_model.dec_v0_wt
        init_head_wt_1 = self.policy_model.dec_v1_wt
        init_head_wt_2 = self.policy_model.dec_v2_wt

        head_wt_0_delta = (init_head_wt_0 - head_wt_0).sum()
        head_wt_1_delta = (init_head_wt_1 - head_wt_1).sum()
        head_wt_2_delta = (init_head_wt_2 - head_wt_2).sum()

        # softmaxed_wt_0 = F.softmax(self.accelerator.unwrap_model(self.policy_model.model).lm_head.weight[lm_head_default_dim*0:lm_head_default_dim*1,:],dim=-1)
        # softmaxed_wt_1 = F.softmax(self.accelerator.unwrap_model(self.policy_model.model).lm_head.weight[lm_head_default_dim*1:lm_head_default_dim*2,:],dim=-1)
        # softmaxed_wt_2 = F.softmax(self.accelerator.unwrap_model(self.policy_model.model).lm_head.weight[lm_head_default_dim*2:lm_head_default_dim*3,:],dim=-1)
        ...
        if self.args['logging']['wandb_log'] and self.accelerator.is_main_process:
            ...
            if step % self.args['logging']['log_interval'] == 0:
                wandb.log({
                    ...
                    # 'train/lm_head_wts/kl_div_0_1': lm_head_kl_div_0_1, 
                    # 'train/lm_head_wts/kl_div_1_2': lm_head_kl_div_1_2,
                    # 'train/lm_head_wts/kl_div_0_2': lm_head_kl_div_0_2,
                    'train/head_wts/var_0': head_wt_0.var(),
                    'train/head_wts/var_1': head_wt_1.var(),
                    'train/head_wts/var_2': head_wt_2.var(),
                    'train/head_wts/delta_0': head_wt_0_delta,
                    'train/head_wts/delta_1': head_wt_1_delta,
                    'train/head_wts/delta_2': head_wt_2_delta,
                    # 'train/lm_head_wts/delta_0': torch.nn.PairwiseDistance(p=2)(init_lm_head_wt_0.reshape(-1),lm_head_wt_0.reshape(-1)),
                    # 'train/lm_head_wts/delta_1': torch.nn.PairwiseDistance(p=2)(init_lm_head_wt_1.reshape(-1),lm_head_wt_1.reshape(-1)),
                    # 'train/lm_head_wts/delta_2': torch.nn.PairwiseDistance(p=2)(init_lm_head_wt_2.reshape(-1),lm_head_wt_2.reshape(-1)),
                    ...
                    'train/reward/verbosity': np.mean(reward_verbosity),
                    'train/reward/factuality': np.mean(reward_factuality),
                    'train/reward/completeness': np.mean(reward_completeness),
                })
    ...
    def valid(self, step):
        ...
        with torch.no_grad():
            ...
            for i, batch in enumerate(tqdm(self.eval_dataloader) if self.eval_dataloader is not None else {}):
                ...
                forward_inputs = {'prompts_input_ids': results['prompts_input_ids'],
                                  'prompts_attention_mask': results['prompts_attention_mask'],
                                  'generated_input_ids': results['generated_input_ids'],
                                  'generated_attention_mask': results['generated_attention_mask'],
                                 }
                ...
                eval_results = self.reward_model.eval_metrics(
                    ...
                    generated_attention_mask=results['generated_attention_mask'],
                    generated_texts=results['generated_text'],
                    metadata = batch['metadata']*self.vector_RL,
                )

                eval_results['eval/logprobs'] = results['generated_logprobs'].mean(dim=-1)
                # import pdb; pdb.set_trace()
                # gather all results
        ...
        if self.accelerator.is_main_process:
            ...
            #................................................uncomment below to save/update chkpts.......................................#
